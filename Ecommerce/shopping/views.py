...
def home(request):
    ...
    new=data[0:1]
    cat=cat[3:4]
    d={"allcat":allcat,'mycart':mycartdata(request.user),'fg':fg,'data':data,"cat":cat,"new":new}
...
def prodetail(request,pid):
    ...
    d = {"allcat": allcat,'detail':pro,'p':p,'size':sizelist,'mycart':mycartdata(request.user)}
